...
export const CharacterController = ({
  ...
  const rotationTarget = useRef(0); // left as-is for compatibility
  ...
  // Camera orbit state
  const cameraAngles = useRef({ yaw: Math.PI, pitch: -0.2, distance: 8 }); // yaw default behind
  ...
  const [nearestGarbage, setNearestGarbage] = useState({
    idx: null,
    dist: 999,
  });
  ...
  // mouse handlers: left-drag to orbit, wheel to zoom
  useEffect(() => {
    const onMouseDown = (e) => {
    ...
    const onMouseMove = (e) => {
      ...
      // sensitivity: tweak these numbers if you want faster/slower rotation
  ...
  // E key action (unchanged)
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.code !== "KeyE" || !rb.current) return;
      const pos = rb.current.translation();
      // If carrying -> try dustbin first
      if (carriedIdx !== null) {
        if (nearestDustbin.dist < 2) {
          onGarbageRemoved(carriedIdx); // remove permanently
          setCarriedIdx(null);
        } else {
          // drop near player
      ...
      // If not carrying -> pick nearest garbage
      if (nearestGarbage.idx !== null && nearestGarbage.dist < 1.5) {
        ...
        if (gRef) {
          ...
          gRef.setTranslation({ x: 0, y: -1000, z: 0 }, true); // hide
          setCarriedIdx(idx);
        }
      }
    };
  ...
  // frame loop: movement + camera follow + checks
  useFrame(({ camera }) => {
    if (rb.current) {
      ...
      // jumping
      const isGrounded = rb.current.translation().y <= 1.1; // close to floor
      if (get().jump && isGrounded) {
        rb.current.applyImpulse({ x: 0, y: 0.08, z: 0 }, true);
      }
      // camera vectors (camera-forward projected to XZ and right)
      ...
      if (movementInput.z !== 0)
        moveWorld.addScaledVector(camForward, movementInput.z);
      if (movementInput.x !== 0)
        moveWorld.addScaledVector(camRight, movementInput.x);

      // set animation based on input & grounded
      ...
      if (moveWorld.lengthSq() > 0.0001) {
        moveWorld.normalize();
        // apply velocity (preserving vertical speed)
        ...
        const desiredAngle = Math.atan2(moveWorld.x, moveWorld.z);
        characterRotationTarget.current = desiredAngle;
      } else {
        // no movement: slow down horizontal motion
        rb.current.setLinvel({ x: 0, y: prevLin.y, z: 0 }, true);

        // face camera direction when idle (so character follows camera)
      ...
      // --- nearest garbage/dustbin checks (same as before) ---
      ...
      // auto-respawn
      if (cPos.y < -5) {
        console.warn("⚠️ Fell below map, respawning...");
        const s = spawnRef.current;
        rb.current.setTranslation({ x: s.x, y: s.y + 1, z: s.z }, true);
        rb.current.setLinvel({ x: 0, y: 0, z: 0 }, true);
      }

      // optional: keep using rotationTarget for some camera smoothing if you used that elsewhere
      // container rotation smoothing (no change to gameplay)
      ...
      // --- camera positioning (orbit around player) ---
